...
object LoggerQueries {
  ...
  def signInQuery(): Unit ={
    val statement = connection.createStatement()
    statement.execute("use project0;")
    val signInQuery = statement.executeQuery(f"SELECT cl_user, cl_pass FROM project0.sql_admin;")
  }
  ...
  def getAllWebsites(): Unit ={
    ...
    while ( getAllSites.next() ) {
      println(getAllSites.getString(1) + " - " + getAllSites.getString(2) + ", " + getAllSites.getString(3) + ", " + getAllSites.getString(4) + "\n") //testQuery1.getString(5), testQuery1.getString(6))
    }
  }
  ...
  def getCertainWebsite(){
    ...
    var getCertainSites = statement.executeQuery(f"SELECT website, username, password, email FROM project0.sql_user WHERE cl_user = '$TESTusername' AND website = '$userInput4Website';")
    while ( getCertainSites.next() ) {
      println(getCertainSites.getString(1) + " - " + getCertainSites.getString(2) + ", " + getCertainSites.getString(3) + ", " + getCertainSites.getString(4)) //testQuery.getString(5), testQuery.getString(6))
    }
  }
  ...
  def updatePass(): Unit ={
    ...
    statement.execute("use project0;")
    println(msgMain)
    var inputWebsite4Pass = readLine("\nWhat Website are you looking for? ")
  ...
  def getEmail(): Unit ={
    val statement = connection.createStatement()
    println(msgMain)
    var inputWebsite4email = readLine("\nWhat email address are you looking for? ")
    println("\nHere is what I can find:\n")
